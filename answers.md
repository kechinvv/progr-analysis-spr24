...
Алгоритм обходит все узлы CFG. Все, что мы делаем на каждом шаге - добавляем/удаляем используемые/неиспользуемые
переменные.
Таким образом временная сложность O(n*k), где n - число узлов, k - количество переменных в программе (высота решетки).

Для корректного анализа переменных из цикла, мы должны проходить его два раза, но дополнительный обход нескольких узлов
особо не влияет на сложность
