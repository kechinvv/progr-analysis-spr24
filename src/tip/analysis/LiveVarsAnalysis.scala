    ...
    n match {
      ...
      case r: CfgStmtNode =>
        r.data match {
          ...
          case as: AAssignStmt =>
            as.left match {
              case id: AIdentifier => s - id ++ as.right.appearingIds //<--- Complete here
              case _ => ???
            }
          case varr: AVarStmt => s -- varr.appearingIds //<--- Complete here
          case ret: AReturnStmt => s ++ ret.exp.appearingIds //<--- Complete here
          case out: AOutputStmt => s ++ out.exp.appearingIds //<--- Complete here
          case _ => s
        }
      case _ => s
    }
